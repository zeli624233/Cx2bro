...
APP_TITLE = "Cx2bro"
PACKAGE_LIMIT_BYTES = 40 * 1024
BASELINE_PASS_RATIO = 0.75
DEFAULT_PASS_BASIS = "包体不超过 40 KB，且测试成功还原数不低于当前来源基线的 75%。"
...
HELP_ITEMS: dict[str, list[tuple[str, str]]] = {
    ...
    "制作扩展集": [
        ...
        ("编辑当前扩展集信息", "编辑扩展集的会社、游戏名、贡献者、版本、日期和说明。日期由年、月、日三个输入框组成，保存时统一为 xxxx-xx-xx；年份限制 2000-2100，月份 1-12，日期 1-31，个位数会自动补 0。"),
    ...
    "扩展集": [
        ("编号列表", "会社列表和游戏列表会显示 1.、2. 这样的编号。编号只用于界面阅读，内部仍使用原始会社名和游戏名，不会影响选择、校验或安装。"),
        ("扩展集详情", "选中扩展集后，右侧会显示会社、游戏、贡献者、版本、日期、说明、路径、HashSeed，以及 rules.int、manifest.int、测试状态等内容。"),
        ("刷新列表", "重新扫描 Extensions 目录。手动复制、删除或安装扩展集后，可以用它刷新界面。"),
        ("打开目录", "打开扩展集根目录或打开当前选中的会社/游戏扩展集目录，方便手动检查配置文件和规则文件。"),
        ("校验选中扩展集", "检查当前扩展集是否具备必要配置，例如 manifest.int、rules.int、HashSeed 和可展开规则。"),
    ...
    "关于": [
        ...
        ("版本信息", "显示软件名称 Cx2bro、版本号 v1.0.0、运行平台和项目主页。核心用途、主要能力列表也在这里。"),
...
class PublisherMetadataDialog(QDialog):
    def __init__(self, parent: "WorkbenchWindow", metadata: dict[str, str]) -> None:
        ...
        self.summary_edit = QLineEdit(metadata.get("summary", ""))
...
class AboutDialog(QDialog):
    ...
    def show_version(self) -> None:
        ...
        self._set_detail_html(
            ...
            <h2>核心用途</h2>
            <p><b>Kirikiri / XP3</b> 游戏解包→还原可读文件名→制作扩展集复用，一条龙。</p>

            <h2>主要能力</h2>
            <ul>
                <li><b>按扩展集还原：</b>有现成扩展集时，拿它还原资源名。</li>
                <li><b>会社集合撞新作：</b>没有新作的扩展集，拿同会社旧作的规律试一下。</li>
                <li><b>动态 StringHash 收集：</b>边跑游戏边收集 Hash，实时看进度、体积和速度。</li>
                <li><b>传统动态模式：</b>用 CxdecCoreCLI 跑一圈补 Hash 遗漏。</li>
                <li><b>制作扩展集：</b>还原好的结果打成 <code>manifest.int</code> / <code>rules.int</code>，装进扩展集库留着下次用。</li>
            </ul>
    ...
    def show_author(self) -> None:
        ...
        self._set_detail_html(
            ...
            <h2>当前项目主要改进方向</h2>
            <ul>
                <li>增加更完整的图形化工作台流程。</li>
                <li>整合静态 Hash 生成、动态 Hash 收集、资源名还原流程。</li>
                <li>增加扩展集制作、测试、安装与复用机制。</li>
                <li>将动态收集到的 Hash 结果整理为可复用的小型 <code>manifest.int</code> / <code>rules.int</code> 扩展集。</li>
                <li>方便后续对同会社作品进行静态 Hash 复用和资源名还原尝试。</li>
            </ul>
...
class BatchExtractorDialog(QDialog):
    ...
    def __init__(self, parent: "WorkbenchWindow", mode: int, output_dir: Path) -> None:
        ...
        self.setWindowTitle(f"Cx2bro XP3 批量提取 - User\\{mode} / {parent.user_flow_name(mode)}")
        if parent.paths.app_icon.exists():
            self.setWindowIcon(QIcon(str(parent.paths.app_icon)))
        self.resize(860, 620)
...
class ExtensionCatalog:
    ...
    def _load_entry(self, brand: str, game_dir: Path) -> ExtensionEntry:
        ...
        def apply_meta(meta: dict[str, str], prefer_game_display: bool = False) -> None:
            ...
            if raw_brand and raw_brand.lower() != "unknown":
                entry.brand = raw_brand
            game_name = meta.get("gamedisplayname") or meta.get("game") or meta.get("gameid")
            if game_name and (prefer_game_display or entry.game == game_dir.name):
...
class WorkbenchWindow(QMainWindow):
    ...
    def __init__(self, paths: WorkbenchPaths) -> None:
        ...
        # 自动刷新：每 5 秒更新首页流程概览、制作扩展集和扩展集信息
    ...
    def _build_user_page(self) -> QWidget:
        ...
        self.user_game_label = QLabel("游戏")
    ...
    def _build_extension_page(self) -> QWidget:
        ...
        toolbar.addWidget(self._button("刷新列表", self.reload_extensions), 0, 0)
        toolbar.addWidget(self._button("打开扩展集目录", lambda: self.open_path(self.paths.extensions_dir)), 0, 1)
        toolbar.addWidget(self._button("打开选中扩展集", self.open_selected_extension), 0, 2)
        toolbar.addWidget(self._button("校验选中扩展集", self.validate_selected_extension), 1, 0)
        toolbar.addWidget(self._button("运行环境自检", self.show_environment_check), 1, 1)
        toolbar.addWidget(self._button("扩展集说明", self.show_extension_description), 1, 2)
        for column in range(3):
        ...
        game_panel = QVBoxLayout()
        game_panel.addWidget(QLabel("游戏"))
    ...
    def update_user_extension_interaction(self) -> None:
        ...
        if hasattr(self, "user_game_label"):
            self.user_game_label.setText("游戏（仅查看）" if company_collection_mode else "游戏")
    ...
    def selected_user_extension_cards(self) -> list[tuple[str, list[str] | str]]:
        ...
        return [
            ("扩展集状态", [
                ...
                f"通过依据：{validation.get('pass_basis') or DEFAULT_PASS_BASIS}",
    ...
    def show_publisher_status(self) -> None:
        ...
        status_lines = self.extension_detail_lines_for_path(draft) if draft and draft.exists() else []
        status_lines.append((
            "功能介绍",
            [
                "制作扩展集",
        ...
        status_lines.append((
            ...
            [
                ...
                f"测试状态：{'通过' if validation_passed == 'yes' else ('未通过' if validation_passed == 'no' else '未测试')}",
                f"通过依据：{validation_basis or DEFAULT_PASS_BASIS}",
        ...
        self.set_info_cards(
            ...
            status_lines + [
                ...
                ("下一步建议", ["先生成当前游戏扩展集，确认并保存游戏名、会社、贡献者等信息；然后测试当前扩展集，确认成功率有底后再安装到扩展集库。"]),
            ],
        )
    ...
    def show_extension_default_info(self) -> None:
        self.set_info_cards(
            ...
            [
                ...
                ("按钮说明", ["刷新列表：重新扫描 Extensions 目录。", "打开扩展集目录：打开全部扩展集所在目录。", "打开选中扩展集：打开当前选中的会社/游戏扩展集目录。", "校验选中扩展集：检查该扩展集是否包含必要文件。", "运行环境自检：检查核心程序、DLL、扩展集目录是否正常。", "扩展集说明：查看扩展集作用和目录结构说明。"]),
            ],
        )
    ...
    def extension_file_count_lines(self, entry: ExtensionEntry) -> list[str]:
        ...
        return [
            ...
            f"测试状态：{validation.get('passed_text') or '未测试'}",
            f"通过依据：{validation.get('pass_basis') or DEFAULT_PASS_BASIS}",
            f"成功率：{validation.get('success_rate') or '未测试'}",
    ...
    def extension_detail_lines_for_path(self, path: Path, brand: str = "", game: str = "") -> list[tuple[str, list[str]]]:
        ...
        validation_lines = [
            ...
            f"通过依据：{pass_basis or DEFAULT_PASS_BASIS}",
    ...
    def _show_publisher_test_popup(self, passed: bool, restored: int, total: int, pass_basis: str) -> None:
        """制作扩展集测试完成后弹出结果窗口，包含各目录还原情况。"""
        lines: list[str] = [
            f"测试结果：{'通过' if passed else '未通过'}",
            f"成功还原：{restored} / {total}，成功率 "
            f"{restored / max(total, 1) * 100:.2f}%",
            f"通过依据：{pass_basis}",
        ]
        # 各目录还原情况
        ...
        box.setText("扩展集测试" + ("通过" if passed else "未通过"))
        box.setInformativeText("\n".join(lines))
        box.addButton("确定", QMessageBox.AcceptRole)
        box.exec()
    ...
    def generate_and_install_minimal_extension(self) -> None:
        ...
        package_bytes = self.publisher_package_size(draft)
        package_limit = PACKAGE_LIMIT_BYTES
        if package_bytes > package_limit:
            self.set_info(
                "当前信息\n\n"
                "最小扩展集未安装。\n\n"
                f"原因：生成结果体积超过限制。\n"
                f"当前大小：{package_bytes} bytes\n"
                f"限制大小：{package_limit} bytes"
            )
            self.log(f"已拦截安装：扩展集大小 {package_bytes} bytes，超过 {package_limit} bytes。")
            return
        ...
        self.show_cli_summary(code, output)
        if code != 0:
            self.log("扩展集验证失败，已停止安装。")
            return

        values = self.parse_cli_values(output)
        validated_restored = int(values.get("RESTORED_FILES", "0") or "0")
        validated_total = int(values.get("TOTAL_FILES", "0") or "0")
        baseline_restored = baseline.get("restored", 0)
        baseline_total = baseline.get("total", 0)
        baseline_threshold = int(baseline_restored * BASELINE_PASS_RATIO + 0.9999) if baseline_restored else 0
        if baseline_threshold and validated_restored < baseline_threshold:
            self.set_info(
                "当前信息\n\n"
                "最小扩展集未安装。\n\n"
                "原因：验证结果低于当前基线的 75%。\n"
                f"当前验证：{validated_restored} / {validated_total}\n"
                f"当前基线：{baseline_restored} / {baseline_total}\n"
                f"通过门槛：{baseline_threshold}\n"
                f"基线报告：{baseline.get('report', '')}"
            )
            self.log(
                "已拦截安装：验证结果低于基线 75%。"
                f" 验证 {validated_restored}/{validated_total}，"
                f" 门槛 {baseline_threshold}，基线 {baseline_restored}/{baseline_total}。"
            )
            return
        ...
        if self.last_imported_extension:
            self.set_info(
                ...
                f"验证结果：{validated_restored} / {validated_total}\n"
                f"通过门槛：{baseline_threshold}（基线 {baseline_restored} 的 75%）"
            )
            self.log(
                "已完成一键生成并安装。"
                f" 大小 {package_bytes} bytes，验证 {validated_restored}/{validated_total}，"
                f" 基线 {baseline_restored}/{baseline_total}。"
            )
    ...
    def test_current_publisher_extension(self) -> None:
        ...
        baseline = self.best_restore_baseline(source_workspace)
        package_bytes = self.publisher_package_size(draft)
        package_limit = PACKAGE_LIMIT_BYTES
        ...
        self.publisher_test_result_written = False
        self.log("已打开原生扩展集测试窗口，主程序保持可操作。")
        result_path = draft / "PublisherTestResult.ini"
        report_path = source_workspace / "Restored_Extractor_Output" / "RestoreReport.txt"
        ...
        try:
            ...
            pass
        if self.publisher_test_poller:
            self.publisher_test_poller.stop()
            self.publisher_test_poller.deleteLater()
        self.publisher_test_poller = QTimer(self)
        self.publisher_test_poller.setInterval(500)

        def poll_report() -> None:
            if result_path.exists() or self.validation_section_exists(manifest) or self.validation_section_exists(rules):
                restore_rules_int_backup()
                self.publisher_test_result_written = True
                self.last_publisher_draft = draft
                self.last_publisher_test_error = ""
                self.show_publisher_status()
                self.log("已检测到 C++ 写入扩展集测试结果，右侧制作状态已刷新。")
                if self.publisher_test_poller:
                    self.publisher_test_poller.stop()
                    self.publisher_test_poller.deleteLater()
                    self.publisher_test_poller = None
                return
            ...
            if values.get("RESTORE") != "completed" and report_path.exists():
                try:
                    fresh_report = report_path.stat().st_mtime + 0.25 >= test_started_at
                except OSError:
                    fresh_report = False
                if fresh_report:
                    counts = self.parse_restore_report_counts(report_path)
                    if counts.get("total", 0):
                        values = {
                            "RESTORE": "completed",
                            "TOTAL_FILES": str(counts.get("total", 0)),
                            "RESTORED_FILES": str(counts.get("restored", 0)),
                        }
            if values.get("RESTORE") == "completed":
        ...
        self.publisher_test_poller.timeout.connect(poll_report)

        def finished(code: int, _status: QProcess.ExitStatus) -> None:
            output = bytes(process.readAllStandardOutput()).decode("utf-8", errors="replace")
            restore_rules_int_backup()
            if not self.publisher_test_result_written:
        ...
        if not process.waitForStarted(3000):
            ...
            self.publisher_test_poller.start()
        return
    ...
    def finish_publisher_extension_test(
        ...
        if code != 0:
            ...
            self.write_publisher_validation_result(
                ...
                draft,
                {
                    ...
                    "PackageLimitBytes": str(package_limit),
                    "TotalFiles": "0",
                    "RestoredFiles": "0",
                    "SuccessRate": "-",
                    "BaselineTotalFiles": str(baseline.get("total", 0)),
                    "BaselineRestoredFiles": str(baseline.get("restored", 0)),
                    "Passed": "no",
                    "PassBasis": "测试进程返回失败，未达到安装条件。",
        ...
        values = self.parse_cli_values(output)
        self.finish_publisher_extension_test_from_values(values, manifest, rules, draft, baseline, package_bytes, package_limit)
    ...
    def finish_publisher_extension_test_from_values(
        ...
        validated_restored = int(values.get("RESTORED_FILES", "0") or "0")
        validated_total = int(values.get("TOTAL_FILES", "0") or "0")
        baseline_restored = int(baseline.get("restored", 0) or 0)
        baseline_total = int(baseline.get("total", 0) or 0)
        passed_size = package_bytes <= package_limit
        baseline_threshold = int(baseline_restored * BASELINE_PASS_RATIO + 0.9999) if baseline_restored else 0
        passed_baseline = not baseline_threshold or validated_restored >= baseline_threshold
        passed = passed_size and passed_baseline
        pass_basis = (
            f"包体 {package_bytes / 1024:.1f} KB / 上限 {package_limit / 1024:.0f} KB：{'通过' if passed_size else '未通过'}；"
            f"成功还原 {validated_restored} / 门槛 {baseline_threshold or 0}（基线 {baseline_restored or 0} 的 75%）：{'通过' if passed_baseline else '未通过'}。"
        )
        self.write_publisher_validation_result(
            manifest,
            rules,
            draft,
            {
                ...
                "SuccessRate": self.restore_rate_text(str(validated_restored), str(validated_total)),
                "BaselineTotalFiles": str(baseline_total),
                "BaselineRestoredFiles": str(baseline_restored),
                "Passed": "yes" if passed else "no",
                "PassBasis": pass_basis,
                "BaselineThresholdFiles": str(baseline_threshold),
                "BaselinePassRatio": str(BASELINE_PASS_RATIO),
                "Error": "" if passed else "测试结果低于基线 75% 或包体超过限制。",
            },
        )
        self.last_publisher_test_error = "" if passed else "测试结果低于基线 75% 或包体超过限制。"
        QApplication.beep()
        self.last_publisher_draft = draft
        self.show_publisher_status()
        self.log(
            f"当前扩展集测试{'通过' if passed else '未通过'}："
            f" 验证 {validated_restored}/{validated_total}，"
            f" 基线 {baseline_restored}/{baseline_total}，"
            f" 大小 {package_bytes} bytes。"
        )
        self._show_publisher_test_popup(passed, validated_restored, validated_total, pass_basis)
    ...
    def import_extension_draft(self) -> None:
        ...
        manifest = draft / "manifest.int"
        rules = draft / "rules.int"
        validation_info = self.extension_validation_info(draft)
        validation_passed = validation_info.get("passed", "") == "yes"
        ...
        if not validation_passed:
            QMessageBox.information(self, APP_TITLE, "当前扩展集还没有测试通过。请先点击“测试当前扩展集”，确认成功率有底后再安装。")
            return
    ...
    def read_draft_metadata(self, draft: Path | None = None) -> dict[str, str]:
        ...
        for path in (draft / "manifest.int", draft / "rules.int"):
            ...
            metadata["brand"] = metadata["brand"] or values.get("brand", "")
            metadata["game"] = metadata["game"] or values.get("game", "")
            metadata["game"] = metadata["game"] or values.get("gamedisplayname", "")
            metadata["game"] = metadata["game"] or values.get("gameid", "")
            metadata["author"] = metadata["author"] or values.get("contributor", "") or values.get("author", "")
            metadata["version"] = metadata["version"] or values.get("version", "")
            file_date = values.get("date", "") or values.get("builddate", "")
            if file_date:
                metadata["date"] = normalize_metadata_date(file_date)
            metadata["summary"] = metadata["summary"] or values.get("summary", "")
        return metadata
    ...
    def apply_publisher_metadata_fields(self, metadata: dict[str, str]) -> None:
        ...
        self.publisher_summary.setText(metadata.get("summary", ""))
    ...
    def write_publisher_validation_result(
        ...
        result_lines = [
            ...
            f"PACKAGE_LIMIT_BYTES={result.get('PackageLimitBytes', '0')}",
            f"BASELINE_RESTORED_FILES={result.get('BaselineRestoredFiles', '0')}",
            f"BASELINE_THRESHOLD_FILES={result.get('BaselineThresholdFiles', '0')}",
        ...
        lines = [
            ...
            f"当前基线: {result.get('BaselineRestoredFiles', '0')} / {result.get('BaselineTotalFiles', '0')}",
            f"基线报告: {result.get('BaselineReport', '')}",
            "",
