...
# 制作扩展集
- 步骤0：使用传统模式解包游戏的资源，并确保还原率不低于75%（不要问为什么，我设定的，扩展集至少能还原大部分的资源，否则他的意义何在呢？（步骤3））
- 步骤1：生成当前游戏的扩展集。
- 步骤2：编辑当前扩展集信息。（可编辑的有：会社名，游戏名称，贡献者，版本，日期，扩展集说明。）
- 步骤3：测试当前扩展集（点一下，软件会自动验证，扩展集是否能用于游戏资源的还原，并统计出还原率，自动记录到扩展集里面）
- 步骤4（可选，校验用）：打开当前扩展集目录（这里有两个目录：游戏exe目录\Publisher\ExtensionDraft和游戏exe目录\Publisher\ExtensionDraft\会社名\游戏名，这里默认打开的是后者）
