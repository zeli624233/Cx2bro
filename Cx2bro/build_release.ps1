...
# 同步到外部 编译版本 目录（用户习惯路径）
$OuterDeployDir = "E:\workbench\gpt5\编译版本\Cx2bro"
$OuterDeployRoot = "E:\workbench\gpt5\编译版本"
cmd /c "rmdir /s /q `"$OuterDeployDir`" 2>nul"
if (-not (Test-Path -LiteralPath $OuterDeployRoot)) {
    New-Item -ItemType Directory -Path $OuterDeployRoot -Force | Out-Null
}
# 用 robocopy 替代 Copy-Item（更可靠，不缓存文件）
robocopy "$AppDir" "$OuterDeployDir" /E /IS /R:0 /W:0 /NFL /NDL /NJH /NJS > $null 2>&1
if ($LASTEXITCODE -le 7) {
    Write-Host "已同步到: $OuterDeployDir ✅"
} else {
    Write-Host "同步到 $OuterDeployDir 失败，可能需要手动复制!"
}
