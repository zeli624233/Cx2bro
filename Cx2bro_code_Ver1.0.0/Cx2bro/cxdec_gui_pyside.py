...
APP_TITLE = "Cx2bro"
PACKAGE_LIMIT_BYTES = 60 * 1024
BASELINE_PASS_RATIO = 0.70
DEFAULT_PASS_BASIS = "包体不超过 60 KB，且测试成功还原数不低于当前来源基线的 70%。"
...
class WorkbenchWindow(QMainWindow):
    ...
    def generate_and_install_minimal_extension(self) -> None:
        ...
        if self.last_imported_extension:
            self.set_info(
                ...
                f"通过门槛：{baseline_threshold}（基线 {baseline_restored} 的 70%）"
            )
            self.log(
                "已完成一键生成并安装。"
    ...
    def finish_publisher_extension_test_from_values(
        ...
        pass_basis = (
            ...
            f"成功还原 {validated_restored} / 门槛 {baseline_threshold or 0}（基线 {baseline_restored or 0} 的 70%）：{'通过' if passed_baseline else '未通过'}。"
        )
        self.write_publisher_validation_result(
            manifest,
            rules,
            draft,
            {
                ...
                "Error": "" if passed else "测试结果低于基线 70% 或包体超过限制。",
            },
        )
        self.last_publisher_test_error = "" if passed else "测试结果低于基线 70% 或包体超过限制。"
        QApplication.beep()
        self.last_publisher_draft = draft
        self.show_publisher_status()
        self.log(
