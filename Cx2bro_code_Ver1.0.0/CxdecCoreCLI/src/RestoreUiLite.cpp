            ...
            case WM_APP_RESTORE_DONE:
            {
                ...
                if (finish)
                {
                    ...
                    else
                    {
                        ...
                        std::wstring rate = FormatRestoreRate(finish->result.restoredFiles, finish->result.totalFiles);
                        SetWindowTextW(context->statusText,
                                       (L"还原完成 | 成功率 " + rate + L" | " +
                                        std::to_wstring(finish->result.restoredFiles) + L" / " +
                                        std::to_wstring(finish->result.totalFiles)).c_str());
                        SetWindowTextW(context->window, (L"Cx2bro 资源名还原 - 已完成 | 成功率 " + rate).c_str());
                        std::wstring detail = L"还原完成。\r\n\r\n";
                        detail += L"总文件数: " + std::to_wstring(finish->result.totalFiles) + L"\r\n";
                        detail += L"成功还原: " + std::to_wstring(finish->result.restoredFiles) + L"\r\n";
                        detail += L"成功率: " + rate + L"\r\n";
                        ...
                        // 各目录还原统计（带成功率）
                        detail += L"各目录还原情况:\r\n";
                        for (auto& pair : context->stateByPackage)
                        {
                            const RowState& rs = pair.second;
                            if (rs.total > 0)
                            {
                                std::wstring pkg = pair.first;
                                if (pkg.size() > 40)
                                    pkg = pkg.substr(0, 37) + L"...";
                                std::wstring pkgRate = FormatRestoreRate(rs.current, rs.total);
                                std::wstring line = L"  " + pkg + L": " +
                                          std::to_wstring(rs.current) + L" / " +
                                          std::to_wstring(rs.total) + L"  (" + pkgRate + L")";
                                detail += line + L"\r\n";
                                // 同时写入日志面板
                                AppendLog(context, line, 95u);
                            }
                        }
