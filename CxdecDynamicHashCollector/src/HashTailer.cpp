...
std::vector<std::string> HashTailer::ReadNewLines(uint64_t maxReadSize)
{
    ...
    // 检测编码（首次读取时探测，之后复用结果）
    // 使用成员变量 encoding_ / bomSize_ 持久化，确保后续增量读取也使用正确的编码
    if (!encodingDetected_)
    {
        encoding_ = DetectEncoding(hFile);
        if (encoding_ == FileEncoding::UTF16LE) bomSize_ = 2;
        else if (encoding_ == FileEncoding::UTF16BE) bomSize_ = 2;
        else if (encoding_ == FileEncoding::UTF8)
        {
    ...
    {
        offset_ = 0;
        encodingDetected_ = false;  // 重新探测编码
    }

    if (fileSize <= (offset_ + bomSize_))
    {
        CloseHandle(hFile);
        return lines;
    }
    ...
    // 计算要读取的字节数（maxReadSize=0 时不限制）
    uint64_t toRead = fileSize - offset_;
    if (maxReadSize > 0 && toRead > maxReadSize)
        toRead = maxReadSize;
...
uint64_t HashTailer::FileSize() const
{
    if (!Exists()) return 0;
    HANDLE hFile = CreateFileW(path_.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
                                nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
bool HashTailer::WasTruncated() const
{
    // 文件被删除（offset>0 但文件不存在）→ 视为被截断
    if (offset_ > 0 && !Exists()) return true;
    return offset_ > 0 && FileSize() < offset_;
}
