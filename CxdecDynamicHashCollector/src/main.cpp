...
struct CollectorContext
{
    ...
    int fileTotal = 0;
    int prevDirCount = 0;   // 上次统计时的 Directory 数量
    int prevFileCount = 0;  // 上次统计时的 FileName 数量
    int idleTicks = 0;      // 日志进度无变化的连续 tick 数

    // 运行时
    ULONGLONG startTick = 0;
    bool running = false;   // 收集是否激活
    bool logsLoaded = false; // 现有日志是否已完全读取
    bool speedHackEnabled = false; // 游戏加速（关闭声音，加速画面，10X倍速），默认关闭
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, wchar_t*, int nCmdShow)
{
    ...
    // 加载内嵌图标（IDI_APPICON 来自 AppIcon.rc）
    HICON hAppIcon = (HICON)LoadImageW(GetModuleHandleW(nullptr), MAKEINTRESOURCEW(101), IMAGE_ICON,
                                        32, 32, LR_DEFAULTCOLOR);
    HICON hAppIconSm = (HICON)LoadImageW(GetModuleHandleW(nullptr), MAKEINTRESOURCEW(101), IMAGE_ICON,
...
void StartWithExisting(HWND hwnd, CollectorContext& ctx)
{
    ...
    // 由 PollLogsForStats 在定时器中以 8MB 为一批读取
    // UI 即时显示进度，不阻塞窗口启动
    // "开始游戏"按钮由 OnTimerTick 中的进度检测自动控制
    ...
    bool noLogs = !ctx.dirTailer->Exists() && !ctx.fileTailer->Exists();
    if (noLogs)
    {
        EnableWindow(GetDlgItem(hwnd, IDC_BTN_LAUNCH), TRUE);
        SetStatusText(hwnd, IDC_STAT_STATUS, L"无日志文件，可直接开始游戏");
        AppendLogText(hwnd, IDC_LOG_DIR, L"[系统] 未检测到日志文件，可直接开始游戏");
        AppendLogText(hwnd, IDC_LOG_FILE, L"[系统] 未检测到日志文件，可直接开始游戏");
    }
    else
    {
        SetStatusText(hwnd, IDC_STAT_STATUS, L"正在后台读取旧日志...");
        AppendLogText(hwnd, IDC_LOG_DIR, L"[系统] 发现现有日志，正在后台读取...");
        AppendLogText(hwnd, IDC_LOG_FILE, L"[系统] 发现现有日志，正在后台读取...");
    }
}
...
void StartCollection(HWND hwnd, CollectorContext& ctx, bool fresh)
{
    ...
    else
    {
        ...
        // 如果日志存在，把已有内容全量读入去重集合（传 0 不限制大小）
        if (ctx.dirTailer->Exists())
        {
            ctx.dirTailer->ResetOffset();
            auto lines = ctx.dirTailer->ReadNewLines(0);
        ...
        {
            ctx.fileTailer->ResetOffset();
            auto lines = ctx.fileTailer->ReadNewLines(0);
            for (const auto& line : lines)
...
void OnTimerTick(HWND hwnd, CollectorContext& ctx)
{
    ...
    // === 检测日志进度，控制"开始游戏"按钮 ===
    // 思路：只要日志统计数据在增长，就认为正在加载，禁用按钮
    // 连续多个 tick 无变化后，认为加载完成，启用按钮
    {
        ULONGLONG curDir = ctx.stats->DirectoryCount();
        ULONGLONG curFile = ctx.stats->FileNameCount();
        bool progressUp = (curDir > (ULONGLONG)ctx.prevDirCount) || (curFile > (ULONGLONG)ctx.prevFileCount);

        if (progressUp)
        {
            ctx.prevDirCount = (int)curDir;
            ctx.prevFileCount = (int)curFile;
            ctx.idleTicks = 0;
            if (IsWindowEnabled(GetDlgItem(hwnd, IDC_BTN_LAUNCH)))
            {
                EnableWindow(GetDlgItem(hwnd, IDC_BTN_LAUNCH), FALSE);
                SetStatusText(hwnd, IDC_STAT_STATUS, L"正在读取日志...");
            }
        }
        else
        {
            ctx.idleTicks++;
            // 连续 2 个 tick（约 3 秒）无新数据 → 加载完成
            if (ctx.idleTicks >= 2 && !IsWindowEnabled(GetDlgItem(hwnd, IDC_BTN_LAUNCH)))
            {
                EnableWindow(GetDlgItem(hwnd, IDC_BTN_LAUNCH), TRUE);
                SetStatusText(hwnd, IDC_STAT_STATUS, L"日志读取完毕，可以开始游戏");
                AppendLogText(hwnd, IDC_LOG_FILE, L"[系统] 日志读取完毕，可以开始加载游戏");
            }
        }
    }
...
void PollLogsForStats(CollectorContext& ctx)
{
    ...
    // ===== 检测文件是否被替换/截断 =====
    // 任一日志文件被替换 → 整体清空并全量快速重新加载（不增量读取）
    {
        bool dirTruncated = ctx.dirTailer->WasTruncated();
        bool fileTruncated = ctx.fileTailer->WasTruncated();
        if (dirTruncated || fileTruncated)
        {
            g_dirHashToName.clear();
            g_fileHashToName.clear();
            ctx.stats->Clear();
            ctx.dirTailer->ResetOffset();
            ctx.fileTailer->ResetOffset();
            ctx.idleTicks = 0;  // 文件变了，重置空闲计数，等待重新加载
            SetStatusText(ctx.hwnd, IDC_STAT_STATUS, L"日志已被替换，正在重新加载...");
            AppendLogText(ctx.hwnd, IDC_LOG_DIR, L"[系统] 日志文件已被删除或替换，正在重新加载...");
            AppendLogText(ctx.hwnd, IDC_LOG_FILE, L"[系统] 日志文件已被删除或替换，正在重新加载...");
        }
    }

    // ===== DirectoryHash.log（offset==0 时全量读取，后续 8MB 增量）=====
    bool fullReadDir = ctx.dirTailer->Offset() == 0 && ctx.dirTailer->Exists();
    auto dirLines = ctx.dirTailer->ReadNewLines(fullReadDir ? 0 : HashTailer::MAX_READ_SIZE);
    ...
    // ===== FileNameHash.log（同样：offset==0 时全量，后续增量）=====
    bool fullReadFile = ctx.fileTailer->Offset() == 0 && ctx.fileTailer->Exists();
    auto fileLines = ctx.fileTailer->ReadNewLines(fullReadFile ? 0 : HashTailer::MAX_READ_SIZE);
...
void OnCommand(HWND hwnd, WPARAM wParam, CollectorContext& ctx)
{
    ...
    switch (id)
    {
    case IDC_CHECK_SPEEDHACK:
    {
        ctx.speedHackEnabled = (SendMessageW(GetDlgItem(hwnd, IDC_CHECK_SPEEDHACK), BM_GETCHECK, 0, 0) == BST_CHECKED);
        break;
    }
    case IDC_BTN_LAUNCH:
    {
        ...
        // 传递加速开关到 Dumper（子进程继承环境变量）
        SetEnvironmentVariableW(L"CXDEC_SPEED_HACK", ctx.speedHackEnabled ? L"1" : L"0");
    ...
    case IDC_BTN_EXIT:
        // "返回主界面"：确认后保存 session 并退出进程，让 Python 下次重新启动
        {
            ...
            if (!hPythonWnd)
            {
                // 遍历所有窗口，找标题以 Cx2bro 开头的
                EnumWindows([](HWND hwnd, LPARAM lParam) -> BOOL {
                    wchar_t title[256];
                    if (GetWindowTextW(hwnd, title, 256) > 0)
                    {
                        if (wcsncmp(title, L"Cx2bro", 6) == 0)
                        {
                            *((HWND*)lParam) = hwnd;
                            return FALSE;
                        }
            ...
            if (hPythonWnd)
            {
                ShowWindow(hPythonWnd, SW_SHOWNORMAL);
